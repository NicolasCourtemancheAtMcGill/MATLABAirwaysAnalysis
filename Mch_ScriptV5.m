...
for Foldernumber = 3:42
    ...
    for i_tnum = 3:length(subFolders_2)
        ...
        smallFrequencies_pre = zeros(1,(5*length(sorted_Files_complete)));
        smallAmplitudes_pre = zeros(1,(5*length(sorted_Files_complete)));
        for ii = 1: length(sorted_Files_complete)
            for iii = 1:5
                ...
                smallFrequencies_pre(((ii-1)*5)+iii) = Frequency;
                smallAmplitudes_pre(((ii-1)*5)+iii) = Amplitude;
        ...
        %         -----------freq storing
        C(end+1, 1:7) = {[Dummy{3} ' ' Dummy{5}], 'f', smallFrequencies(1), smallFrequencies(2), smallFrequencies(3), smallFrequencies(4), smallFrequencies(5)};
        
        %         -----------amp storing
        C(end, 8:12) = {smallAmplitudes(1), smallAmplitudes(2), smallAmplitudes(3), smallAmplitudes(4), smallAmplitudes(5)};
